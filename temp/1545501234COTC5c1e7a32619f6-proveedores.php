...
$sql = "SELECT `proveedores`.`id`, `proveedores`.`nit`, `proveedores`.`empresa`, `proveedores`.`nombre`, `proveedores`.`telefono`, `proveedores`.`direccion`, `proveedores`.`correo`, `proveedores`.`pedido`, `proveedores`.`observacion`, `proveedores`.`yrtryyt` FROM `proveedores`   ";
...
$sql .= ' concat(LOWER(`proveedores`.`id`)," ", LOWER(`proveedores`.`nit`)," ", LOWER(`proveedores`.`empresa`)," ", LOWER(`proveedores`.`nombre`)," ", LOWER(`proveedores`.`telefono`)," ", LOWER(`proveedores`.`direccion`)," ", LOWER(`proveedores`.`correo`)," ", LOWER(`proveedores`.`pedido`)," ", LOWER(`proveedores`.`observacion`)," ", LOWER(`proveedores`.`yrtryyt`)," ") LIKE "%'.mb_strtolower($dato, 'UTF-8').'%"';
$cont ++;
...
$sql = "INSERT INTO proveedores (`id`, `nit`, `empresa`, `nombre`, `telefono`, `direccion`, `correo`, `pedido`, `observacion`, `yrtryyt`) VALUES ('".$_POST['id']."', '".$_POST['nit']."', '".$_POST['empresa']."', '".$_POST['nombre']."', '".$_POST['telefono']."', '".$_POST['direccion']."', '".$_POST['correo']."', '".$_POST['pedido']."', '".$_POST['observacion']."', '".$_POST['yrtryyt']."')";
 /*echo $sql;*/
...
echo '<p><label for="yrtryyt">Yrtryyt:</label><input class=""name="yrtryyt"type="text" id="yrtryyt" value="';if (isset($row['yrtryyt'])) echo $row['yrtryyt'];echo '"';echo ' required ></p>';
...
$sql = "SELECT `id`, `nit`, `empresa`, `nombre`, `telefono`, `direccion`, `correo`, `pedido`, `observacion`, `yrtryyt` FROM `proveedores` WHERE id ='".$_POST['cod']."' Limit 1"; 
$consulta = $mysqli->query($sql);
 /*echo $sql;*/ 
...
$sql = "UPDATE proveedores SET id='".$_POST['id']."', nit='".$_POST['nit']."', empresa='".$_POST['empresa']."', nombre='".$_POST['nombre']."', telefono='".$_POST['telefono']."', direccion='".$_POST['direccion']."', correo='".$_POST['correo']."', pedido='".$_POST['pedido']."', observacion='".$_POST['observacion']."', yrtryyt='".$_POST['yrtryyt']."'WHERE  id = '".$cod."';";
/* echo $sql;*/ 
 /*Se conecta a la BD y luego ejecuta la instrucción SQL*/ 
